...
public class Pohon : MonoBehaviourPunCallbacks
{
    [SerializeField]
    public int myHP;
    ...
    void ExplodeRPC()
    {
        transform.eulerAngles = new Vector3(transform.eulerAngles.x,transform.eulerAngles.y,transform.eulerAngles.z + 1.5f);

        AudioSource underAttack = transform.Find("TreeFall").GetComponent<AudioSource>();
        transform.Find("TreeFall").transform.position = transform.position;
    ...
    public void Tumbang()
    {
        ...
        if (random == 1)
        {
            GameObject go = PhotonNetwork.InstantiateSceneObject(Path.Combine("Models/Tree", "Leaf"), new Vector3(transform.position.x + Random.Range(-2f, 2f), transform.position.y + Random.Range(3, 5), transform.position.z + Random.Range(-2f, 2f)), transform.rotation);
            go.name = "Leaf (" + go.GetPhotonView().ViewID + ")";
            go.GetComponent<PhotonView>().RPC("itemrpc", RpcTarget.All, go.name, "Obstacles");
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (PhotonNetwork.IsMasterClient)
            ...
            }else if (collision.collider.name.Equals("Ground"))
            {
                GetComponent<PhotonView>().RPC("setkinematicRPC", RpcTarget.All);                
            }
